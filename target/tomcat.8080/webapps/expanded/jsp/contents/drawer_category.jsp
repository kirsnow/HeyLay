...
<title>분야별 카드 서랍 | Quration: 답을 열어줄 그런 사람</title>
						...
						<c:otherwise>
							<c:forEach var="drawerHeader" items="${ drawerHeaders }" varStatus="loop">
								<div class="row marginTop30">
									<span class="text-primary lead">${ drawerHeader.category } <small>저장해 둔 카드 ${ drawerHeader.cnt } 건</small></span>
								</div>
								<div class="row card-container mdl-grid">
									<c:forEach var="card" items="${ drawerCards }" varStatus="loop">
						                <c:if test="${ card.categoryName eq drawerHeader.category }" >
						                <!-- card -->
