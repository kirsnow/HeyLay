			...
			<div class="modal-body">
				<section>
					...
					<dl>
						...
						<dd class="marginTop">
							<p class="cardInfo drop-text-3">undefined</p>
						</dd>
					</dl>
				</section>
				<section class="text-right">
					<textarea id="memoMessage" class="form-control marginTop" rows="3"
						placeholder="메모 쓰기..." maxlength="4000"></textarea>
					<small class="text-muted"><span
						id="text_length_memoMessage">4,000</span>자 입력 가능</small>
				</section>
			</div>
								...
								<c:if test="${ (dirList ne null) and (not empty dirList) }">
									<c:forEach var="dir" items="${ dirList }" varStatus="loop">
										<li role="presentation">
											<a role="menuitem" tabindex="-1" id="${ dir.no }" 
												class="dirList">${ dir.name }</a>
										</li>
									</c:forEach>
									<li class="divider"></li>
								</c:if>
								<li role="presentation"><a role="menuitem" tabindex="-1"
									id="-1" class="newDirectory">+ 새 폴더</a></li>
							</ul>
						</div>
						<div class="btn-group" role="group">
							<button type="button" id="putCard"
								class="btn btn-primary form-control">담기</button>
						</div>
					</div>
				</div>
			</div>
...
<script>
			...
			$.ajax({
					...
					error : function(result) {
							...
							$('button#putCard')
								...
								.addClass('btn-danger')
								.html(
									'<i class="fa fa-exclamation-circle" aria-hidden="true"></i> 오류'
									);
	...
	$('.saveCancelBtn').on('click',function() {
		...
		$('button.saveCancelBtn').attr('disabled', 'disabled')
		...
		$.ajax({
				...
				success : function(result) {
						...
						// change Btn (content_detail.jsp)
						$('button.saveCancelBtn').hide()
							.removeAttr('disabled').removeClass('btn-warning')
