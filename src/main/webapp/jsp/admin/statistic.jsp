...
<script>
	...
	$.ajax({
		...
		success : function(response) {
			...
			var vis = d3.select("#myGraph4").append("svg:svg").data([ data ])
			...
			arcs.append("svg:text").attr("transform", function(d) {
				...
				return "translate(" + c[0] + "," + c[1] + ")";
					...
					"central").style("font-size", "15px").style(
					"text-decoration", "bold").text(function(d, i) {
				return data[i].label;
			});
			arcs.append("svg:text").attr("transform", function(d) {
				var c = arc.centroid(d);
				return "translate(" + c[0] + "," + c[1] + ")";
			}).attr('dy', '2em').attr("text-anchor", "middle").style(
					"font-size", "12px").style("text-decoration", "bold").text(
					function(d, i) {
						return data[i].value;
					});
		},
	...
	$
			.ajax({
				...
				success : function(response) {
					...
					var colorSet_colorful = [ "#FCE4EC", "#F8BBD0", "#F48FB1",
							"#F06292", "#EC407A" ]; //그린
					var colorSet_blue = [ "#03A9F4", "#BDBDBD", "#E0E0E0",
							"#EEEEEE", "#F5F5F5" ]; //블루 & 그레이
					var color = d3.scale.ordinal().range(colorSet_blue); //컬러 변경
					...
					var vis = d3.select("#myGraph2").append("svg:svg").data(
							[ data ]).attr("width", w).attr("height", h)
					...
					arcs.append("svg:text").attr("transform", function(d) {
						...
						return "translate(" + c[0] + "," + c[1] + ")";
							...
							"font-size", "12px").style("text-decoration",
	...
	$
			.ajax({
				...
				success : function(response) {
					...
					var colorSet_colorful = [ "#C8E6C9", "#A5D6A7", "#81C784",
							"#66BB6A", "#4CAF50" ]; //옐로우
					var colorSet_blue = [ "#03A9F4", "#BDBDBD", "#E0E0E0",
							"#EEEEEE", "#F5F5F5" ]; //블루 & 그레이
					var color = d3.scale.ordinal().range(colorSet_blue); //컬러 변경
					...
					var vis = d3.select("#myGraph").append("svg:svg").data(
							[ data ]).attr("width", w).attr("height", h)
							.append("svg:g").attr("transform",
									"translate(" + r + "," + r + ")");
					var pie = d3.layout.pie().value(function(d) {
